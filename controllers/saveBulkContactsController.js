...
const saveBulkContacts = async (req, res) => {
    try {
        ...
        for (let batchStart = 0; batchStart < contacts.length; batchStart += BATCH_SIZE) {
            ...
            const allEmails = processedBatch.flatMap((c) => c.normalized.emailAddresses || []);

            // Query existing contacts for this user within this batch
            const existingContactsBatch = await Contact.find({
                createdBy: req.user._id,
                $or: [
                    { "phoneNumbers.number": { $in: allNumbers.length ? allNumbers : ["__NONE__"] } },
                    { emailAddresses: { $in: allEmails.length ? allEmails : ["__NONE__"] } },
                ],
            }).lean();
