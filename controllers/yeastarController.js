...
//   const { YEASTAR_BASE_URL, YEASTAR_USER_AGENT } = getConfig();
//   return axios.create({
//     baseURL: YEASTAR_BASE_URL,
//     headers: { "User-Agent": YEASTAR_USER_AGENT, "Content-Type": "application/json" },
//     timeout: 15000,
//   });
// };

// async function login() {
//   const {YEASTAR_USERNAME} = getConfig()
//   // const res = await axiosInstance.post('/login', { username: USER, password: PASS });
//   const res = await getAxiosInstance().post("/login", {
//     // username: USER,
...
//     const res = await getAxiosInstance().post(url, body);
...
//  */
// async function createYeastarExtensionSequential({
//   startFrom = 1001,
//   maxAttempts = 200,
//   name = "",
// } = {}) {
...
//         : JSON.stringify(attempt.error).toLowerCase();
//     if (
...
//       const retryErr =
//         typeof retry.error === "string"
//           ? retry.error.toLowerCase()
//           : JSON.stringify(retry.error).toLowerCase();
...
//   throw new Error(
//     `No available extension found in range ${startFrom}..${startFrom + maxAttempts - 1}`,
//   );
// }
