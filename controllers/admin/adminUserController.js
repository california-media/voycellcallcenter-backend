...
exports.adminRegisterUser = async (req, res) => {
    try {
        ...
        const verificationLink = `https://voycell-call-center-admin-frontend.vercel.app/user-verification?verificationToken=${emailVerificationToken}`;
...
exports.getAllUsersByCompanyAdmin = async (req, res) => {
    try {
        ...
        const users = await User.find({ createdByWhichCompanyAdmin: companyAdminId })
            .select("-password -salt -otp -otpExpiresAt -resetPasswordToken -resetPasswordExpires") // hide sensitive data
            .sort({ createdAt: -1 });
