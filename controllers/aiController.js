...
// Generates 3-4 content variations for email or in-app notification.
// Body: { prompt, type: "email" | "notification" }
...
Return ONLY a valid JSON array of 3 objects. No explanation, no markdown code block.`;
    ...
    // GPT sometimes wraps the array in { "variations": [...] } — unwrap it
    let parsed;
    try {
...
// Proxies call audio transcription + summarization (replaces direct OpenAI in frontend).
// Accepts: multipart with field "audio" (audio file) OR JSON { audioUrl }
exports.transcribeAndSummarize = async (req, res) => {
  try {
    ...
    if (req.file) {
      // Audio uploaded as binary
      const stream = Readable.from(req.file.buffer);
      stream.path = req.file.originalname || "call_audio.mp3";
      audioFile = stream;
    } else if (req.body?.audioUrl) {
      // Fetch from a URL (e.g. S3 presigned URL)
